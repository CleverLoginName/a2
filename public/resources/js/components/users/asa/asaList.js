var ASAList = function (containerId,data) {
    ...
    _editAction : function(id){
        ...
        $.ajax({
        	...
        	if (response && response['type'] === 'SUCCESS' && response['data']) {
                ...
                $('#usr-list-view').hide();
                var asaUser = new ASAView('usr-item-view',response['data'],'usr-list-view');
                asaUser.init();
                $('#usr-item-view').show();
        	...
        	alertify.error("Failed to load Adopto System Administrator due to System Fault.");
        });

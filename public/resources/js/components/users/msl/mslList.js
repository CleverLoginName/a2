var MSLList = function (containerId,data) {
...
MSLList.prototype = {
    ...
    init: function () {
        ...
        window.breadcrumb.addItem("Master Software Licensees", this.container.attr('id'));
    ...
    _editAction : function(id){
        ...
        $.ajax({
        	...
        	if (response && response['type'] === 'SUCCESS' && response['data']) {
                ...
                $('#usr-list-view').hide();
                var mslUser = new MSLView('usr-item-view',response['data'],'usr-list-view');
                mslUser.init();
                $('#usr-item-view').show();
        	...
        	alertify.error("Failed to load Master Software Licensee due to System Fault.");
        });

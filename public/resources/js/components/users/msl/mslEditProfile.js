 ...
 *
 */
var MSLEditProfile = function (containerId,data) {
...
MSLEditProfile.prototype = {
    ...
    _submitData : function(data){
        ...
        $.ajax({
            type: 'POST',
            url: 'restAPI/user/msl/' + _this.data['id'] + '/update',
            data: data,
            processData: false,
            contentType: false,
            cache:false
            ...
            if (response && response['type'] === 'SUCCESS') {
        		...
        		setTimeout(function(){        			 
                    location.href = 'msl-view-profile';
        		}, 200);
        	...
        	alertify.error("Failed to Update Master Software Licensee's profile due to System Fault.");
            $('#' + _this.id + '-edit-user-loading').hide();
            $('#' + _this.id + '-edit-user-btn').attr('disabled',false);
        });

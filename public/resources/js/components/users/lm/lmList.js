var LMList = function (containerId,data) {
...
LMList.prototype = {
    ...
    init: function () {
        ...
        window.breadcrumb.addItem("Merchants", this.container.attr('id'));
    ...
    _editAction : function(id){
        ...
        $.ajax({
        	...
        	if (response && response['type'] === 'SUCCESS' && response['data']) {
                ...
                $('#usr-list-view').hide();
                var lmUser = new LMView('usr-item-view',response['data'],'usr-list-view');
                lmUser.init();
                $('#usr-item-view').show();
        	...
        	alertify.error("Failed to load Merchant due to System Fault.");
        });

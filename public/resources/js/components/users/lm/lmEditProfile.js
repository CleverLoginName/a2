 ...
 *
 */
var LMEditProfile = function (containerId,data) {
...
LMEditProfile.prototype = {
    ...
    _submitData : function(data){
        ...
        $.ajax({
            type: 'POST',
            url: 'restAPI/user/lm/' + _this.data['id'] + '/update',
            data: data,
            processData: false,
            contentType: false,
            cache:false
        	...
        	alertify.error("Failed to Update Merchant's profile due to System Fault.");
            $('#' + _this.id + '-edit-user-loading').hide();
            $('#' + _this.id + '-edit-user-btn').attr('disabled',false);
        });

    ...
    <div class="row" id="prod-list-view">
        <div class="box">
            ...
            <div class="box-body table-responsive">
                {!! Form::open(['url' => 'users/roles', 'method' => 'post', 'class'=>"row new-item-from-wrapper"]) !!}
                            ...
                            @foreach($roles as $role)
                                ...
                                <td>
                                    @foreach($activeUsers as $activeUser)
                                        @if(($activeUser->role_id == $role->id) && ($activeUser->user_id==$user->id))
                                            <?php
                                            echo '<input type="radio" name="user-'.$user->id.'" value="'.$role->id.",".$user->id.'" checked>';
                                            ?>
                                            <?php $notAdded = false; ?>
                                        @endif
                                    @endforeach
                                    @if($notAdded)
                                        <?php
                                        echo '<input type="radio" name="user-'.$user->id.'" value="'.$role->id.",".$user->id.'">';
                                        ?>
                                    @endif
                                </td>
                            @endforeach
                ...
                <section class="row box-footer" id="form-footer">
                    ...
                    <a id="prod-frm-reset" href="{!! url('users/permissions') !!}" class="btn add-item-btn" style="margin-right:10px;">Permissions Management</a>
                </section>
                {!! Form::close() !!}
            </div>
        </div>
...
@section('bread-crumb')
    ...
    <i class="fa fa-chevron-right breadcrumb-icn " id="1-ic"></i>
          <span
                class="breadcrumb-text font-blue">User Role Management</span></button>
    <i class="fa fa-chevron-right breadcrumb-icn " id="1-ic"></i>
